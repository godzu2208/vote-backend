  ...
  try {
    ...
    if (
      [
        ...
        "invalid_duration",
        "invalid_pin",
        "invalid_background_type",
      ].includes(code)
    ) {
      return res
        .status(400)
        .json({ error: code, message: "Dữ liệu Game không hợp lệ." });
    }
    if (
      code === "game_pin_taken" ||
...
router.get("/games", authMiddleware, requireAdmin, async (req, res) => {
  try {
    return res.json({ games: await listGames(req.user!.id) });
  } catch (err) {
...
router.get("/games/pin/:pin", authMiddleware, async (req, res) => {
  ...
  if (!PIN_PATTERN.test(pin))
    return res
      .status(400)
      .json({ error: "invalid_pin", message: "Game PIN không hợp lệ." });
  try {
    ...
    console.error("[GET /games/pin/:pin]", err);
    return res.status(500).json({ error: "internal_error" });
  }
});
...
router.get("/games/:id", authMiddleware, async (req, res) => {
  try {
    const game = await getGame(
      req.params.id,
      req.user!.isAdmin ? req.user!.id : undefined,
    );
    return res.json({ game });
  } catch (err) {
...
router.get("/games/:id/results", authMiddleware, async (req, res) => {
  try {
    const results = await getGamePublicResults(req.params.id);
...
router.post("/games/:id/join", authMiddleware, async (req, res) => {
  try {
    ...
    if (game.status === "closed")
      return res
        .status(409)
        .json({ error: "game_closed", message: "Game đã kết thúc." });
    await joinGame(
      req.params.id,
      req.user!.id,
    ...
    if (err?.code === "invalid_display_name")
      return res
        .status(400)
        .json({ error: err.code, message: "Vui lòng nhập tên hiển thị." });
...
router.get(
  ...
  async (req, res) => {
    try {
      return res.json({
        participants: await getGameParticipants(req.params.id),
      });
    } catch (err) {
      console.error("[GET /games/:id/participants]", err);
      return res.status(500).json({ error: "internal_error" });
    }
  },
);
...
router.get(
  "/games/:id/participants/count",
  authMiddleware,
  async (req, res) => {
    try {
      return res.json({ count: await getParticipantCount(req.params.id) });
    } catch (err) {
      console.error("[GET /games/:id/participants/count]", err);
      return res.status(500).json({ error: "internal_error" });
    }
  },
);
...
router.post(
  ...
  async (req, res) => {
    try {
      return res.json({
        ok: true,
        game: await enterLobby(req.params.id, req.user!.id),
      });
    } catch (err: any) {
...
router.post(
  ...
  async (req, res) => {
    try {
      return res.json({
        ok: true,
        game: await advanceGame(req.params.id, req.user!.id),
      });
    } catch (err: any) {
...
router.post(
  ...
  async (req, res) => {
    try {
      return res.json({
        ok: true,
        game: await closeGame(req.params.id, req.user!.id),
      });
    } catch (err) {
      console.error("[POST /games/:id/close]", err);
      return res.status(500).json({ error: "internal_error" });
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    try {
      return res.json(await getGameDashboard(req.params.id, req.user!.id));
    } catch (err) {
      console.error("[GET /games/:id/dashboard]", err);
      return res.status(500).json({ error: "internal_error" });
    }
  },
);
...
router.get(
  ...
  async (req, res) => {
    try {
      return res.json(await getGameLiveStats(req.params.id, req.user!.id));
    } catch (err) {
      console.error("[GET /games/:id/stats]", err);
      return res.status(500).json({ error: "internal_error" });
    }
  },
);
...
router.get(
  "/games/:id/questions/:questionId/voters",
  authMiddleware,
  requireAdmin,
  async (req, res) => {
    try {
      const voters = await getQuestionVoters(
        req.params.id,
        req.user!.id,
      ...
      console.error("[GET /games/:id/questions/:questionId/voters]", err);
      return res.status(500).json({ error: "internal_error" });
    }
  },
);
